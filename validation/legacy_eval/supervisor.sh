...
readonly REPO=${REPO:?set REPO to the exact checkout path}
readonly RUN_BASE=/data/qwen38-int8-lab/evaluations
readonly CANDIDATE=/data/models/Qwen3.8-27B-W8A8-INT8
readonly SOURCE=/data/models/Qwen3.8-27B-source-download/model
readonly EVAL_IMAGE=${EVAL_IMAGE:-qwen38-int8-lab/eval:0.1.0}
readonly SUITE_CONFIG=${REPO}/validation/legacy_eval/config/leaderboard-v2.yaml
...
write_model_manifests() {
    ...
    python3 "${REPO}/validation/legacy_eval/scripts/checkpoint_manifest.py" "${CANDIDATE}" --output "${candidate_file}"
    python3 "${REPO}/validation/legacy_eval/scripts/checkpoint_manifest.py" "${SOURCE}" --output "${source_file}"
...
prefetch_and_validate() {
    ...
    if ! docker run "${container_common[@]}" --env HF_TOKEN --entrypoint python "${EVAL_IMAGE}" \
        /app/validation/legacy_eval/scripts/prefetch.py --output /run/dataset-preflight.json; then
    ...
    docker run "${container_common[@]}" --env HF_DATASETS_OFFLINE=1 --env HF_HUB_OFFLINE=1 \
        --entrypoint python "${EVAL_IMAGE}" /app/validation/legacy_eval/scripts/run_harness.py validate --tasks leaderboard
    CURRENT_STAGE=request_preflight
    host_preflight
    docker run "${container_common[@]}" --env HF_DATASETS_OFFLINE=1 --env HF_HUB_OFFLINE=1 \
        --entrypoint python "${EVAL_IMAGE}" /app/validation/legacy_eval/scripts/request_preflight.py \
...
run_guarded_command() {
    ...
    [[ -z ${trigger} ]] || return 1
    set +e
    scan_reason=$(python3 "${REPO}/validation/legacy_eval/scripts/runtime_log_check.py" --log "${log}" --failure-only 2>/dev/null)
    scan_status=$?
    set -e
...
run_runtime_gate() {
    ...
    command=(
        ...
        --entrypoint /bin/bash "${EVAL_IMAGE}" /app/validation/legacy_eval/scripts/container_eval.sh
        python /app/validation/legacy_eval/scripts/runtime_loglikelihood_gate.py
    ...
    if ! EVAL_SUITE_CONFIG="${SUITE_CONFIG}" python3 "${REPO}/validation/legacy_eval/scripts/runtime_log_check.py" \
...
run_eval_stage() {
    ...
    command=(
        ...
        --env EVAL_OUTPUT_UID="$(id -u)" --env EVAL_OUTPUT_GID="$(id -g)"
        --entrypoint /bin/bash "${EVAL_IMAGE}" /app/validation/legacy_eval/scripts/container_eval.sh
        python /app/validation/legacy_eval/scripts/run_harness.py run
    ...
    python3 - "${result_file}" "${REPO}/validation/legacy_eval/config/leaderboard-v2.yaml" "${stage_name}" <<'PY'
...
for group in mmlu_pro bbh gpqa math_hard ifeval musr; do
    harness_task=$(python3 - "${REPO}/validation/legacy_eval/config/leaderboard-v2.yaml" "${group}" <<'PY'
import sys, yaml
...
verify_manifests_unchanged
finalize_telemetry ''
python3 "${REPO}/validation/legacy_eval/scripts/aggregate.py" --run-root "${RUN_ROOT}" \
