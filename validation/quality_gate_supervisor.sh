...
readonly SOURCE_MODEL=/data/models/Qwen3.8-27B-source-download/model
...
preflight() {
    ...
    (( disk >= MIN_DISK_BYTES ))
    [[ ! -e "${destination}" ]]
    revision=$(python3 - "${REPO}/recipes/int8-original-v1/quant.yaml" <<'PY'
import sys, yaml
...
validate_and_serve() {
    ...
    sudo --user=emmy -- docker run --rm --entrypoint python \
        ...
        qwen38-int8-lab/vllm:0.1.0 /app/validation/validate_quant.py /candidate
    ...
    sudo --user=emmy -- python3 "${REPO}/validation/smoke_test.py" \
        --output "${smoke}"
    if [[ "${do_benchmark}" == true ]]; then
        [[ ! -e "${host_model}/EXPERIMENTAL_NON_PRODUCTION.json" ]]
        sudo --user=emmy -- python3 "${REPO}/validation/benchmark.py" --output "${benchmark}"
