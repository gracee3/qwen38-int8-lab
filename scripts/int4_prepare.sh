...
command=(); extra=(); image=$quant; memory=6g
case "$stage" in
    ...
    real-pilot|full)
        ...
        (( free_bytes >= 64*1024*1024*1024 )) || { echo 'BLOCKED: require 64 GiB disk headroom'; exit 2; }
...
echo "log=$log"
docker run "${common[@]}" --network "$network" --memory "$memory" --memory-swap "$memory" "${extra[@]}" --entrypoint /usr/bin/env "$image" "${command[@]}" 2>&1 | tee "$log"
