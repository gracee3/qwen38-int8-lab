...
stage=${1:?usage: bash quant/int4.sh audit|synthetic|runtime|corpus|screen-candidate|finalize|screen-final|real-pilot|full}
resume=${2:-}
...
common=(--rm --cpus 2 --shm-size 256m
    ...
    --mount "type=bind,src=${SOURCE_MODEL:-/data/models/Qwen3.8-27B-source-download/model},dst=/models/source,readonly"
...
guard=()
case "$stage" in
    audit) command=(python /app/validation/audit_int4_targets.py /models/source /app/recipes/int4-expanded400-v1/quant.yaml --hash-shards --output /run-int4/target-audit-hashed.json);;
    synthetic) extra=(--gpus "device=$gpu"); command=(python /app/validation/int4_synthetic.py --output /run-int4/synthetic);;
    runtime)
        image=$runtime
        ...
        command=(python /app/validation/int4_runtime_pilot.py);;
    corpus)
        extra=(--mount 'type=bind,src=/data/qwen38-int8-lab/cache/huggingface,dst=/hf,readonly')
        command=(python /app/calibration/prepare_int4_corpus.py --cache /hf --source /models/source --output /run-int4/calibration-candidate);;
    screen-candidate|screen-final)
        ...
        if [[ $stage == screen-candidate ]]; then
            ...
            network=bridge
            command=(bash -ec 'python /app/calibration/screen_int4_overlap.py; python /app/calibration/freeze_int4_canaries.py; python /app/calibration/review_int4_origins.py')
        else
            command=(bash -ec 'python /app/calibration/screen_int4_overlap.py --corpus calibration-final --output overlap-final-public.json; python /app/calibration/freeze_int4_canaries.py --corpus calibration-final --output overlap-final-agents.json; python /app/calibration/review_int4_origins.py --corpus calibration-final --output origin-final-schema.json; python /app/calibration/assemble_int4_clearance.py')
        fi;;
    finalize) command=(python /app/calibration/finalize_int4_corpus.py);;
    real-pilot|full)
        ...
        memory_args=()
        guard=(bash "$repo/quant/with_quant_swappiness.sh")
        extra=(--gpus "device=$gpu")
        command=(python /app/quant/quantize_int4.py --audit /run-int4/target-audit-hashed.json --runtime-report /run-int4/runtime-result.json --corpus /run-int4/calibration-final)
        if [[ $stage == full ]]; then
