...
quant resume="false":
    bash "{{repo_root}}/scripts/with_quant_swappiness.sh" just --justfile "{{repo_root}}/Justfile" _quant "{{resume}}"
...
v2-quant resume="false":
    bash "{{repo_root}}/scripts/with_quant_swappiness.sh" just --justfile "{{repo_root}}/Justfile" _v2-quant "{{resume}}"

_v2-quant resume:
    mkdir -p "{{work_root}}/logs"; log="{{work_root}}/logs/v2-quant-$(date -u +%Y%m%dT%H%M%SZ).log"; echo "v2 full quant log: $log"; docker run --rm --gpus all --ipc=host --mount type=bind,src="{{model_root}}",dst=/models --mount type=bind,src="{{source_model}}",dst=/models/source,readonly --mount type=bind,src="{{work_root}}",dst=/work --mount type=bind,src="{{repo_root}}",dst=/app,readonly -e HF_HOME=/work/cache/huggingface -e HF_DATASETS_CACHE=/work/cache/huggingface/datasets -e GIT_COMMIT="$(git -C "{{repo_root}}" rev-parse HEAD)" -w /app "{{quant_image}}" python /app/quant/scripts/quantize.py --config /app/quant/config/qwen38-27b-v2.yaml --profile quality --output /models/Qwen3.8-27B-W8A8-INT8-Agentic-v2 --execute-full {{ if resume == "true" { "--resume" } else { "" } }} 2>&1 | tee "$log"
...
serve-vllm-tp2-fp8-160k:
    test -x "{{cuda_toolkit_root}}/bin/nvcc"; "{{cuda_toolkit_root}}/bin/nvcc" --version | grep -F 'release 13.3'; mkdir -p "{{work_root}}/logs"; log="{{work_root}}/logs/vllm-tp2-fp8-160k-$(date -u +%Y%m%dT%H%M%SZ).log"; echo "vLLM TP2/FP8 log: $log"; docker run --rm --name qwen38-vllm-tp2-fp8-160k --gpus all --ipc=host -p "127.0.0.1:{{port}}:8000" --mount type=bind,src="{{model_root}}",dst=/models,readonly --mount type=bind,src="{{work_root}}",dst=/work --mount type=bind,src="{{repo_root}}",dst=/app,readonly --mount type=bind,src="{{cuda_toolkit_root}}",dst=/usr/local/cuda,readonly -e CUDA_HOME=/usr/local/cuda -e VLLM_CACHE_ROOT=/work/cache/vllm -e VLLM_USE_FLASHINFER_SAMPLER=0 "{{vllm_fp8_image}}" /models/$(basename "{{output_model}}") --served-model-name qwen38-w8a8-tp2-fp8 --api-key "{{api_key}}" --tensor-parallel-size 2 --pipeline-parallel-size 1 --max-model-len 163840 --kv-cache-memory-bytes 3221225472 --kv-cache-dtype fp8 --cpu-offload-gb 0 --seed 42 --language-model-only --enable-prefix-caching --enable-chunked-prefill --max-num-batched-tokens 2048 --max-num-seqs 1 --enable-auto-tool-choice --tool-call-parser qwen3_xml --default-chat-template-kwargs '{"enable_thinking":false}' --generation-config vllm --no-enable-log-requests --disable-uvicorn-access-log 2>&1 | tee "$log"
